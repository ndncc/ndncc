...
main (int argc, char *argv[])
{
  ...
  std::ifstream fin;
  fin.open("src/ndnSIM/examples/topologies/bcube-4-3-flow-mutable");
  ...
  while(fin>>consumer>>producer>>no_packets)
  {
	  ...
	  Ptr<MyApp> app = CreateObject<MyApp> ();
		app->Setup (serversocket, clientAddr, 1140, no_packets, DataRate ("100Mbps"));	//0 means infinite packets
