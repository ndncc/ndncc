...
main (int argc, char *argv[])
{
  ...
  ndnGlobalRoutingHelper.AddOrigins ("/prefix2", nodes.Get (3));
  ...
  consumers.Stop (Seconds (200));
  
  consumerHelper.SetPrefix ("/prefix2");
  consumerHelper.SetAttribute ("NackFeedback", StringValue ("1.1")); 
  consumerHelper.SetAttribute ("InitLimit", StringValue ("1.0"));
  consumers = consumerHelper.Install (nodes.Get (1)); // first node
  consumers.Start (Seconds (100));	
  consumers.Stop (Seconds (200)); 
